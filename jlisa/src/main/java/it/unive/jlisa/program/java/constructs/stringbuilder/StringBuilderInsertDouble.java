...
public class StringBuilderInsertDouble extends TernaryExpression implements PluggableStatement {
	protected Statement originating;

	public StringBuilderInsertDouble(
			CFG cfg,
	...
	public static StringBuilderInsertDouble build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringBuilderInsertDouble(cfg, location, params[0], params[1], params[2]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdTernarySemantics(
		...
		AnalysisState<A> result = StringBuilderMutationSupport.mutateValue(analysis, state, left, stringType,
				...
				oldValue -> new it.unive.lisa.symbolic.value.TernaryExpression(
						stringType, oldValue, middle, right, JavaStringInsertDoubleOperator.INSTANCE, getLocation()));
