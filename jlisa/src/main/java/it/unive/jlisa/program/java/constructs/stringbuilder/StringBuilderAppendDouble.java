...
public class StringBuilderAppendDouble extends BinaryExpression implements PluggableStatement {
	protected Statement originating;

	public StringBuilderAppendDouble(
			CFG cfg,
	...
	public static StringBuilderAppendDouble build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringBuilderAppendDouble(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		AnalysisState<A> result = StringBuilderMutationSupport.mutateValue(analysis, state, left, stringType,
				...
				oldValue -> new it.unive.lisa.symbolic.value.BinaryExpression(
						stringType, oldValue, right, JavaStringAppendDoubleOperator.INSTANCE, getLocation()));
