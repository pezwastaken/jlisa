...
public class StringBuilderAppendLong extends BinaryExpression implements PluggableStatement {
	protected Statement originating;

	public StringBuilderAppendLong(
			CFG cfg,
	...
	public static StringBuilderAppendLong build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringBuilderAppendLong(cfg, location, params[0], params[1]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdBinarySemantics(
		...
		AnalysisState<A> result = StringBuilderMutationSupport.mutateValue(analysis, state, left, stringType,
				...
				oldValue -> new it.unive.lisa.symbolic.value.BinaryExpression(
						stringType, oldValue, right, JavaStringAppendLongOperator.INSTANCE, getLocation()));
