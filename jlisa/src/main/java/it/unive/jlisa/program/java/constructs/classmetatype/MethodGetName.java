...
public class MethodGetName extends it.unive.lisa.program.cfg.statement.UnaryExpression implements PluggableStatement {
	...
	public static MethodGetName build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new MethodGetName(cfg, location, params[0]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdUnarySemantics(
		...
		AccessChild accessThisMethodName = new AccessChild(new JavaReferenceType(stringType), derefThisMethod, nameVar,
				getLocation());

		HeapReference ref = new HeapReference(reftype, accessThisMethodName, getLocation());
