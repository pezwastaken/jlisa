...
public class StringBuilderInsertLong extends TernaryExpression implements PluggableStatement {
	protected Statement originating;

	public StringBuilderInsertLong(
			CFG cfg,
	...
	public static StringBuilderInsertLong build(
			CFG cfg,
			CodeLocation location,
			Expression... params) {
		return new StringBuilderInsertLong(cfg, location, params[0], params[1], params[2]);
	}
	...
	public <A extends AbstractLattice<A>, D extends AbstractDomain<A>> AnalysisState<A> fwdTernarySemantics(
		...
		AnalysisState<A> result = StringBuilderMutationSupport.mutateValue(analysis, state, left, stringType,
				...
				oldValue -> new it.unive.lisa.symbolic.value.TernaryExpression(
						stringType, oldValue, middle, right, JavaStringInsertLongOperator.INSTANCE, getLocation()));
